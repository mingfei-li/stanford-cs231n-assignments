...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    next_h = h0
    for i in range(T):
        next_h, cache_step = rnn_step_forward(x[i], next_h, Wx, Wh, b)
        h.append(next_h)
        cache['step_caches'].append(cache_step)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dx[i], dh_prev, dWx_step, dWh_step, db_step = rnn_step_backward(dh[i] + dh_prev, step_caches[i])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
    dh0 = dh_prev
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = x, prev_h, prev_c, Wx, Wh, i, f, o, g, y
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, i, f, o, g, y = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    next_c = np.zeros((N, H))
    for i in range(T):
        next_h, next_c, step_cache = lstm_step_forward(x[i], next_h, next_c, Wx, Wh, b)
        h.append(next_h)
        cache['step_caches'].append(step_cache)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dprev_h += dh[i]
        dx[i], dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(dprev_h, dprev_c, step_caches[i])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
    dh0 = dprev_h
