...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            if i != k:
                denom_k += np.exp(sim(z_k, out[i]) / tau)
            if i != k + N:
                denom_k_N += np.exp(sim(z_k_N, out[i]) / tau)
        
        total_loss += -2*sim(z_k, z_k_N) / tau + np.log(denom_k) + np.log(denom_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    norm_left = torch.linalg.norm(out_left, dim=1, keepdims=True)
    norm_right = torch.linalg.norm(out_right, dim=1, keepdims=True)
...
def compute_sim_matrix(out):
    ...
    col = out.unsqueeze(1)
    row = out.unsqueeze(0)
    col_norm = torch.linalg.norm(col, dim=-1)
    row_norm = torch.linalg.norm(row, dim=-1)
    sim_matrix = torch.sum(col*row, dim=-1) / (col_norm * row_norm)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos = torch.cat([sim_pos, sim_pos], dim=0).squeeze(1)
    ...
    loss = torch.sum(-log_numerator+torch.log(denom)) / (2*N)
